"""E-SEQ image saves keep songs and regenerate only their matching catalog."""
...
@pytest.mark.parametrize("save_route", ("modified", "repacked", "commit"))
def test_eseq_saves_filter_final_payloads_and_catalogs(tmp_path, variant, save_route):
    ...
    saved_session = None
    try:
        if save_route == "commit":
            session.commit_to_source(**changes)
            ...
            output = session.create_modified_image(**changes)
...
def test_directory_generation_itself_removes_unlisted_payloads(tmp_path):
    ...
    try:
        ...
        assert {entry.path for entry in session.list_entries().entries} == {"NOEXT", "PIANODIR.FIL"}
